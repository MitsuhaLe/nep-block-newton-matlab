...
# compute_MM computes the residual of the invariant pair (S,X). It's a function in nep-pack
# logger is an log logger
# SPMF: Sum of Products of Matrices and Functions
# The following defines an anonymous function using the matrix 2-norm
...
function blocknewton1(nep::AbstractSPMF;
                     S::Matrix=zeros(2,2),
                     X::Matrix=Matrix(1.0I, size(nep,1), 2),
                     errmeasure::Function =  default_block_errmeasure(nep::NEP),
                     tol::Real=eps(real(eltype(S)))*100,   # eltype element type
                     maxit::Integer=10,
                     logger=0,
    ...
    # Note that here the code's author directly set l=p (theoretically, the minimality index l<=p)
    WW=zeros(T,n,p,p)
    for j=1:size(W,2)
        WW[:,:,j] = W[(j-1)*n .+ (1:n), :]
    end
    ...
    for k=1:maxit
        ...
        # By default, armijo_factor=1，i.e. not use armijo rule
        if (armijo_factor<1)
