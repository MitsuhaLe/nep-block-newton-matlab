...
% Input: A  - cell containing the matrices A_j of the NLEVP
...
n = size(A{1},1); m = size(A,2); k = size(X,2); l = size(W,3);
...
for i = 1:k
    ...
    T11 = zeros(n); 
    for j = 1:m 
        T11 = T11 + A{j}*feval(f,j,s);     % matrix T(s)
    end
    
    T12 = zeros(n,k);
    for j = 1:m
        DF = feval(f,j,[S, eye(k); zeros(k) s*eye(k)]);
        T12 = T12 + A{j}*X*DF(1:k,k+1:2*k);  % [Df(S)]_ii
    end
    ...
    for j = 1:m
        DF = feval(f,j, [S, Z; zeros(k) S]);   % [Df(S)](Z)
        RT(:,i+1:k) = RT(:,i+1:k) - A{j} * ( dX(:,i)*fS(i,i+1:k,j) ...
            + X*DF(1:k,k+i+1:2*k) );
    end
    
    for j = 2:l
        RV(:,i+1:k) = RV(:,i+1:k) - W(:,:,j)' * ( dX(:,i)*pS(i,i+1:k,j-1) ...
            + X*DS(:,i+1:k) );
        DS = DS*S + pS(:,:,j-1)*DS;
    end
end